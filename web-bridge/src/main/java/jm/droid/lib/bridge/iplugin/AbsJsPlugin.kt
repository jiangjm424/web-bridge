 ...
 */
abstract class AbsJsPlugin(
    protected val activity: AppCompatActivity,
    private val jsApiProxy: JsApiProxy?,
) : JsInvokeBridge {
    var errMsg: String? = null

    private val methods = mutableMapOf<String, (JsCommonReq) -> String>()
    private val _methods: Map<String, (JsCommonReq) -> String> = methods
    ...
    abstract fun onConfig(methods: MutableMap<String, (JsCommonReq) -> String>)
    override fun dispatchJsEvent(
        method: String,
        request: JsCommonReq,
    ): String {
        return _methods[method]?.invoke(request)?.apply {
            request.callbackId?.let { callback ->
                notifyJs(callback, this)
            }
        } ?: JsApiResult.fail(
            JsErrCode.ERR_NO_METHOD,
            request.reqId,
            "no method found",
        )
    }

    protected fun notifyJs(method: String, resp: String?) {
        jsApiProxy?.callJs(method, resp ?: empty)
    }
